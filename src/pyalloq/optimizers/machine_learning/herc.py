...
class HERCAllocator(HRPAllocator):
    ...
    def _get_cluster_risk(self, cov: np.ndarray, cluster: list) -> float:
        ...
        erc_result = self.erc_solver.allocate(cov_matrix=df_sub_cov)
        w_erc = np.asarray(erc_result.weights.values)
        sub_cov_val = df_sub_cov.values
        cluster_variance = float(w_erc.T @ sub_cov_val @ w_erc)

        return cluster_variance

    def _recursive_bisection(
        self, cov: np.ndarray, sort_ix: np.ndarray, weights: pd.Series
    ) -> pd.Series:
